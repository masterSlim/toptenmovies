...
public class ParserService implements MovieService {
    ...
    public LinkedList<MovieEntity> getMovies(int... years) throws IOException {
        ...
        if(page == null){
            throw new RuntimeException("Страница с фильмами не найдена");
        }
     ...
     * Название css-классов блока-маркера и остальных блоков информации указывается вручную в конфигурационном файле.
     *
     * @param page Объект, представляющий собой страницу, в которой будет производиться поиск информации о фильмах.
    ...
    private LinkedList<MovieEntity> parse(Document page) {
        ...
        for (int i = 0; i < titleEl.size(); i++) {
             ...
             * Моим решением было взять за основу 2 записи, которые присутствуют в строке всегда - год и длительность,
